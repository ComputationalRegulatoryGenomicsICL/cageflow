...
# All commented code lines were removed unless it was uncommented by the modifier.
...
.fit.power.law.to.reverse.cumulative <- function(values, val.range = c(10, 1000)) {
    ...
    if(any(val.range < 1)){
        stop(
            paste(
                "The range of values for fitting the power law",
    ...
    if(nrow(v) < 1){
        stop(
            paste(
                "Selected range for fitting the power law does not contain any",
