...
* `--dist N` sets an optical duplicate distance `N` to remove optical duplicates, in addition to PCR duplicates (see [`samtools markdup`, option `-d`](https://www.htslib.org/doc/samtools-markdup.html)); the argument optional and requires `--dedup`.
