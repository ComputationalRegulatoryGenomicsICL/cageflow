...
6. Map the trimmed reads onto the Bowtie2 index using [`bowtie2`](https://bowtie-bio.sourceforge.net/bowtie2/manual.shtml) and [`samtools view`](https://www.htslib.org/doc/samtools.html) with options `-b -F 4 -q 20` to filter out unmapped reads and select only uniquelly mapped reads.
...
The pipeline has toy *S. cerevisiae* CAGE data stored in [assets/sacer_fq](https://github.com/ComputationalRegulatoryGenomicsICL/customcageq/tree/dev/assets/sacer_fq) for testing purposes (single-end reads in the [se](https://github.com/ComputationalRegulatoryGenomicsICL/customcageq/tree/dev/assets/sacer_fq/se) subfolder and paired-end reads in [pe](https://github.com/ComputationalRegulatoryGenomicsICL/customcageq/tree/dev/assets/sacer_fq/pe) subfolder). The single-end reads were obtained by subsampling from ([Börlin et al., 2018](https://academic.oup.com/femsyr/article/19/2/foy128/5257840)), [ERR2495152](https://www.ebi.ac.uk/ena/browser/view/ERR2495152), while the paired-end reads were generated by taking subsample of the paired-end library from ([Chabbert et al., 2015](https://www.embopress.org/doi/full/10.15252/msb.20145776)), [SRR1631657](https://www.ebi.ac.uk/ena/browser/view/SRR1631657) and saving them as "forward" and "reverse" reads.
