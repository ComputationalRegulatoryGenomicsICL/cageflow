...
To run the pipeline on these data, use the following command:
...
adapting the suggested values for the options `-profile` and `-w` according to your system's setup.

Paired-end test reads were randomly sampled from zebrafish CAGE libraries "4-5 somites" (SRR10215487) and "prim-5" (SRR10215486) generated and analyzed in ([Nepal et al., 2020](https://doi.org/10.1038/s41467-019-13687-0)). The test data includes two samples split into two lanes each. One lane contains approximately 1 mln reads, thus the whole dataset contains around 4 mln reads. See `generate_pe_test_data.sh` for details on how the test dataset was generated.
